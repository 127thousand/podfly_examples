...
import 'package:azure_rt_client/azure_rt_client.dart';
...
///
/// On Flutter web in the Azure monolith image, the UI and API share one origin
/// (Container Apps → nginx → Serverpod). Prefer that over baked-in localhost / placeholders.
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Serverpod realtime on Azure Container Apps',
      theme: ThemeData(
...
class _HomePageState extends State<HomePage> {
  final _nameController = TextEditingController(text: 'Azure');
  String? _hello;
  String? _helloError;
  final _ticks = <String>[];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Serverpod · Azure Container Apps'),
      ...
      body: ListView(
        ...
        children: [
          Text(
            'Monolith on Azure Container Apps: nginx serves Flutter web and '
            'proxies API + WebSockets to Serverpod on localhost:8081. '
            '(Unlike AWS App Runner, Container Apps supports WebSocket upgrades.)',
          ...
          ..._ticks.map(
            (t) => Card(
              child: ListTile(
                dense: true,
                leading: const Icon(Icons.bolt, color: Color(0xFF0078D4)),
