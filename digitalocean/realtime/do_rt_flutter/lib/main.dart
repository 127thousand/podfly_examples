...
import 'package:do_rt_client/do_rt_client.dart';
...
Future<String> resolveServerUrl() async {
  ...
  if (kIsWeb) {
    ...
    // DO web app is separate from API app — never use web origin for streams.
    final onDo = Uri.base.host.contains('ondigitalocean.app');
    if (isLocalOrPlaceholder && !onDo) {
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Serverpod realtime on DigitalOcean',
      theme: ThemeData(
...
class _HomePageState extends State<HomePage> {
  final _nameController = TextEditingController(text: 'DigitalOcean');
  String? _hello;
  String? _helloError;
  final _ticks = <String>[];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Serverpod · DigitalOcean realtime'),
      ...
      body: ListView(
        ...
        children: [
          Text(
            'App Platform: API app + web app (nginx). RPC and WebSocket streams '
            'hit the API host (SERVER_URL), not the static web origin.',
