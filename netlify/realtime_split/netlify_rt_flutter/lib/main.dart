...
import 'package:netlify_rt_client/netlify_rt_client.dart';
...
///
/// Split deploy: Flutter is on Netlify; Serverpod (HTTP + WebSockets) is on Fly.
/// Prefer baked-in SERVER_URL / config; only fall back to same-origin for local
/// monolith-style dev (not Netlify production).
Future<String> resolveServerUrl() async {
  ...
  if (kIsWeb) {
    ...
    // On Netlify, Uri.base is the CDN — streams must hit Fly instead.
    final onNetlify = Uri.base.host.contains('netlify.app');
    if (isLocalOrPlaceholder && !onNetlify) {
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      title: 'Serverpod realtime on Fly + Netlify',
      theme: ThemeData(
...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Serverpod · Fly + Netlify'),
      ...
      body: ListView(
        ...
        children: [
          Text(
            'Split deploy: Flutter UI is on Netlify; Serverpod API and '
