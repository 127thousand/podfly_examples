...
import 'package:hetzner_rt_client/hetzner_rt_client.dart';
...
///
/// On Flutter web in the Hetzner monolith image, the UI and API share one origin
/// (Hetzner VPS → nginx → Serverpod). Prefer that over baked-in localhost / placeholders.
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Serverpod realtime on Hetzner Cloud',
      theme: ThemeData(
        colorScheme: ColorScheme.fromSeed(seedColor: const Color(0xFFD50C2D)),
        useMaterial3: true,
      ),
      home: const HomePage(),
    );
...
class _HomePageState extends State<HomePage> {
  final _nameController = TextEditingController(text: 'Hetzner');
  String? _hello;
  String? _helloError;
  final _ticks = <String>[];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Serverpod · Hetzner Cloud'),
      ...
      body: ListView(
        ...
        children: [
          Text(
            'Monolith on Hetzner Cloud: nginx serves Flutter web and '
            'proxies API + WebSockets to Serverpod on localhost:8081. '
            '(Unlike AWS App Runner, Hetzner VPS supports WebSocket upgrades.)',
          ...
          ..._ticks.map(
            (t) => Card(
              child: ListTile(
                dense: true,
                leading: const Icon(Icons.bolt, color: Color(0xFFD50C2D)),
