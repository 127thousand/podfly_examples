...
import 'package:ghp_rt_client/ghp_rt_client.dart';
...
///
/// Split deploy: Flutter is on GitHub Pages; Serverpod (HTTP + WebSockets) is on Fly.
/// Prefer baked-in SERVER_URL / config; only fall back to same-origin for local
/// monolith-style dev (not GitHub Pages production).
Future<String> resolveServerUrl() async {
  ...
  if (kIsWeb) {
    ...
    // On GitHub Pages, Uri.base is the CDN — streams must hit Fly instead.
    final onGitHubPages = Uri.base.host.contains('github.io');
    if (isLocalOrPlaceholder && !onGitHubPages) {
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      title: 'Serverpod realtime on Fly + GitHub Pages',
      theme: ThemeData(
...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Serverpod · Fly + GitHub Pages'),
      ...
      body: ListView(
        ...
        children: [
          Text(
            'Split deploy: Flutter UI is on GitHub Pages; Serverpod API and '
